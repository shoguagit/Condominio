...
class ConciliacionRepository:
    ...
    def sugerir_vinculacion(
        ...
        if (mov.get("tipo") or "").lower() != "ingreso":
            return None
        pagos = self.obtener_pagos_periodo(condominio_id, periodo_db)
        return sugerir_vinculacion_desde_filas(mov, pagos)

    @safe_db_operation("conciliacion.obtener_pagos_periodo")
    def obtener_pagos_periodo(self, condominio_id: int, periodo_db: str) -> list[dict]:
        return (
            self.client.table(self._pag)
